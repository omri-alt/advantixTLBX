...
def post_bid_factor(camp_id, sub_id, bid_factor):
    """
    POST /affiliate/v2/campaigns/{id}/bid-factor with JSON ``{"subId", "bidFactor"}``.

    Live-verified: ``bidFactor`` may be ``0`` to stop buying from that sub-publisher
    (API returns 200 with ``{"subId": "...", "bidFactor": 0.0}``). Values between 0
    and the minimum effective bid can return 400 (\"Bid rate can not be lower than $0.01\").
    """
    endpoint = f"https://api.sourceknowledge.com/affiliate/v2/campaigns/{camp_id}/bid-factor"
    data = {"subId": str(sub_id), "bidFactor": float(bid_factor)}
    r = requests.post(endpoint, headers=headers_sk, json=data)
    return r
...
def update_bid_factor2(camp_id, list):
    ...
    if r.status_code == 429:
        print(
            f"pause for 60 sec for rate limit exceeded and status code is {r.status_code}"
        )
        time.sleep(60)
        r = requests.post(endpoint, headers=headers_sk, json=data)
    elif r.status_code != 200:
        print(
            f"error in updating bid factor for campaign {camp_id} and status code is {r.status_code} and response is {r.json()}"
        )
    print(r.json())
    return
...
def optimize_newsource_fix7days2():
    updated = []
    fix = get_fixcamp()
    print(f"Running KLFIX optimization at time {datetime.now().strftime('%H:%M:%S')}")
    for camp in fix:
        ...
        if len(newsub) != 0:
            ...
            #print(toupdate)
    try:
        print(f'Ran KLFIX optimization from campaign {fix[0]["id"]} to {fix[-1]["id"]} and updated {len(updated)} campaigns at time {datetime.now().strftime("%H:%M:%S")}')
    except:
        print(f'Ran KLFIX optimization and updated {len(updated)} campaigns at time {datetime.now().strftime("%H:%M:%S")} but there are no active campaigns')
    return updated
        ...
        if stats['itemsCount'] == 0:
            ...
            for item in stats['items']:
                if item['subId'] == source:
                    ...
                    #print(f"source {source} found in campaign {camp['id']} with winrate {item['winRate']} and bidfactor {item['bidFactor']}")
...
def campsOfTodayRename():
    ...
    for camp in camps:
        if camp['start'][0:10] == today and camp['advertiser']['name'].split(
            ...
            if int(camp['advertiser']['id']) > 102588:
                newcamps.append(camp)
                data = camp['advertiser']
                data['name'] = data['name'].replace('KLFIX', 'KLWL8')
                print(data['name'])
