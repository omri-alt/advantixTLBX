...
def stage_monthly_log(ctx: RunContext) -> int:
    ...
    try:
        ...
        print(f"   Kelkoo link checks: feed1={n1}, feed2={n2} (yesterday={ctx.yesterday_str})")
        if n1 == 0 and n2 == 0:
...
def stage_pla_offers(ctx: RunContext) -> int:
    ...
    ctx.write_json_artifact("offers_meta.json", {"offers_1": offers_1, "offers_2": offers_2, "rows1": len(rows1), "rows2": len(rows2)})
    return 0
