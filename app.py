...
DAILY_POSTBACK_FEED_KEYS = frozenset(
    list(KELKOO_POSTBACK_FEED_TAGS) + ["adexa", "yadore", "yadore_sales", "effinity", "shopnomix"]
)
...
def ui_workflow(workflow_key: str):
    ...
    if request.method == "POST":
        ...
        if workflow_key == "blend":
            ...
            if bf in ("kelkoo1", "kelkoo2", "kelkoo5", "kelkoo4", "adexa", "yadore", "both", "all"):
                # Avoid duplicate --feed if user pasted one in the text box; dropdown wins.
                extra_args = re.sub(
                    r"--feed\s+(kelkoo1|kelkoo2|kelkoo5|kelkoo4|adexa|yadore|both|all)\b",
                    "",
                    extra_args,
                    flags=re.IGNORECASE,
                )
...
def api_overview_slice_kelkoo2_api():
    """
    Kelkoo feed2 collectible MTD (aggregated lead revenue × FEED2 share).

    Cache hit is instant; miss rebuilds synchronously via aggregated reports (~5–15s).
...
def api_overview_slice_kelkoo2_api_refresh():
    """Rebuild feed2 API revenue (fast aggregated; returns completed payload)."""
    try:
        return jsonify(queue_slice_kelkoo2_api_revenue_refresh()), 200
    except Exception as e:
