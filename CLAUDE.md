...
**No-Val-click skip (merchant pick):** Yesterday’s Nipuhim offers (NIPUHIM-feed1/2/5) that received clicks (`NO_VAL_CLICK_MIN_CLICKS`, default 5) but **0 Val_clicks** (`valid_clicks` / `custom_conversion_7`) are aggregated per `(feed, geo, merchant)`. If that merchant is chosen again today, it is dropped and the next ranked fixim merchant fills the slot (`NO_VAL_CLICK_RANK_BUFFER` extra candidates). Manual `--merchant-override` ids are kept. Disable with `NO_VAL_CLICK_ENABLED=0`. Preview: `python cli/no_val_click.py`.
...
**Control Center (Flask):** `/workflows/daily` exposes geo + merchant-mode dropdowns; they normalize args so users do not have to type raw flags. The homepage loads `GET /api/postback-status` (UTC “today” rollup from `runtime/daily_postbacks_last_run.json`) for the slim postback banner above the overview. Homepage overview tiles include affiliation revenue, DSP costs, and net. **Affiliation revenue** is Keitaro conversion revenue by monetization network (1st of UTC month through yesterday; `affiliation_revenue` on `GET /api/overview`); the top Revenue and Net tiles use that same total. The affiliation card has its own **Refresh** (same API rebuild as **Refresh from APIs**). After deploy, `OVERVIEW_SNAPSHOT_BOOTSTRAP` (default `missing`) rebuilds the overview if the file is absent or schema-stale (no affiliation block) and fills missing domain-demand / Blend-cap caches (`always` = all snapshots once). Cron: `python cli/refresh_dashboard_snapshots.py`. **Daily workflow v2** is scheduled at **09:00 Asia/Jerusalem** (`DAILY_WORKFLOW_SCHEDULER_*`; skips if already running or succeeded today). **Kelkoo daily postbacks** are scheduled hourly **08:00–23:00 Asia/Jerusalem** (`KELKOO_DAILY_POSTBACK_SCHEDULER_*`; first try at 08:00, then hourly until geos are done; startup catch-up if 08:00 was missed): probe raw-report API per geo, process ready countries, hourly retry for missing geos (log: `data/kelkoo_daily_postbacks_run_log.json`). Kelkoo 2 and 4 postbacks apply **0.7** net (`FEED2_POSTBACK_REVENUE_SHARE` / `FEED4_POSTBACK_REVENUE_SHARE`); feed 5 applies **0.85** (`FEED5_POSTBACK_REVENUE_SHARE`). **Adexa + Yadore click postbacks** at **12:30 Asia/Jerusalem** (`ADEXA_YADORE_DAILY_POSTBACK_SCHEDULER_*`; log: `data/adexa_yadore_daily_postbacks_run_log.json`). Hub tiles show yellow/green/red day status. **Long-form UI copy** (flags, caveats, examples) lives in `**/help`** (Help Center); tool pages keep a one-line subtitle + `?` link to the matching anchor.
...
- **7a¾ — Blend no-Val-click highlight:** rows whose Keitaro Blend offers had yesterday clicks and 0 Val_clicks are colored light red (`#f4cccc`); other data rows are reset to white. Does not skip or remove Blend merchants. Apply: `python cli/no_val_click.py --apply-blend-colors`.
